...
// Định nghĩa props cho CommentItem
...
const CommentItem: React.FC<CommentItemProps> = ({ item }) => {
    return (
        <View style={styles.commentContainer}>
                ...
                <View style={{ flex: 1 }}>
                    <Text >
                        <Text style={styles.commentUser}>
                            {item.user}</Text> rated it {Array.from({ length: 5 }, (_, index) => (
                                <Text key={index} style={styles.commentStar} >{item.rating ? (index < item.rating ? '★' : '☆') : '☆'}
                                </Text>
                            ))}
                    </Text>
            ...
            <View >
                <View style={{ alignItems: 'flex-end', marginTop: 10 }}><LikeButton initialLikes={item.like} /></View>
            </View>
